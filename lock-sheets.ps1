...
function Invoke-LockSheets {
    param(
        [Parameter(Mandatory = $true)]
        [string]$WorkbookPath
    )

    Write-Log "Starting sheet lock"
    ...
    try {
        ...
        for ($n = $rangeStart; $n -le $rangeStop; $n++) {
            ...
            if ($sheet.ProtectContents) {
                Write-Log "Sheet '$sheetName' already protected, skipped" -Level WARNING
                $skipped++
                continue
            }
        ...
        $workbook.Save()
        Write-Log "Workbook saved successfully ($locked locked, $skipped skipped)"
        Write-Log "Sheet lock completed for $WorkbookPath"
    }
    catch {
...
foreach ($path in $workbookPaths) {
    ...
    try {
        Invoke-LockSheets -WorkbookPath $path
    }
    catch {
...
Write-Log "All sheet locks completed successfully"
